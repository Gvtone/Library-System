...
namespace Library_System
{
    public partial class UserRegister : Form
    {
                ...
                if (ShowException())
                {
                    ...
                    else
                    {
                        ...
                        if (txtPass.Text == txtPassCheck.Text)
                        {
                            ...
                            {
                                reader.Close();
                                Com.CommandText = $"INSERT INTO users(Username, Password, Email, User_Type) VALUES('{txtUser.Text}', '{encrypt(txtPass.Text)}', '{txtEmail.Text}', 'USER')";
                                Com.ExecuteNonQuery();
                                CN.Close();

                                if (MessageBox.Show("Account successfuly created!", "Save", MessageBoxButtons.OK, MessageBoxIcon.Information) == DialogResult.OK)
                                {
        ...
        // Option to register an admin account
        private void lblRegForAdmin_MouseEnter(object sender, EventArgs e)
        {
            lblRegForAdmin.ForeColor = Color.FromArgb(134, 185, 176);
        }

        private void lblRegForAdmin_MouseLeave(object sender, EventArgs e)
        {
            lblRegForAdmin.ForeColor = Color.White;
        }

        private void lblRegForAdmin_Click(object sender, EventArgs e)
        {
